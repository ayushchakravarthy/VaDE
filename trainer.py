...
class Trainer:
    ...
    def train_GMM(self):
        ...
        print('Fitting GMM')
        # might not work
        x = torch.cat([data[0] for data in self.dataloader]).view(-1, 784).to(self.device)
    ...
    def save_weights_VaDE(self):
        ...
        self.VaDE.pi_prior_data = torch.from_numpy(self.gmm.weight_).float().to(self.device)
        self.VaDE.mu_prior_data = torch.from_numpy(self.gmm.means_).float().to(self.device)
        self.VaDE.log_var_prior_data = torch.log(torch.from_numpy(self.gmm.covariances_)).float().to(self.device)
